 ...
 *
 * The master plan's Part 2.2 describes minting $STACKD ourselves and then
 ...
 * Rewards Vault is funded by claiming creator trading fees (Part 2.5 item 1 —
 ...
 * which is exactly the safety property Part 2.4 was designed around.
 */
...
/** Part 2.3: graduate at 750 USDC of quote raised. Mainnet value, always. */
...
/**
 * Part 2.2's 15% team/liquidity bucket, expressed the only way DBC supports it:
 ...
 * automatic refill source (Part 2.5 item 1), so it must not be zero.
 */
export const CREATOR_TRADING_FEE_PERCENTAGE = 50;

/** Fixed trading fee in bps. No time-decay scheduler, per Part 2.3. */
 ...
 * is the "gentle/linear" shape Part 2.3 asks for — it rewards sustained demand
...
export function buildStackdCurveConfig(cluster: 'devnet' | 'mainnet' = 'devnet') {
  ...
  return buildCurve({
    token: {
      ...
      // Part 2.2: no mint authority may survive genesis. Immutable is how DBC
    ...
    fee: {
      baseFeeParams: {
        ...
        // Part 2.3 explicitly wants no anti-bot decay: the only way onto this
    ...
    migration: {
      ...
      migrationFeeOption: MigrationFeeOption.FixedBps100, // 100 bps, Part 2.3
...
// ---------------------------------------------------------------------------
// Pricing — replaces the Day 3 placeholder
// ---------------------------------------------------------------------------
